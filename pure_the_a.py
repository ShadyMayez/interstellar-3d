...
RENDER_OUT = '/home/shady/Downloads/interstellar/the_a_pure_preview.png'
...
print(f"Initial verts: {len(mesh.vertices)}")
...
for v in bm.verts:
    # Delete outer square border plate (abs(X) > 35 or abs(Z) > 35 or Y < -2.0)
...
print(f"Loose components: {len(loose)}")
...
for o in loose:
    ...
    print(f"  '{o.name}': verts={len(o.data.vertices)}, X=({min(xs):.1f}..{max(xs):.1f}), Y=({min(ys):.1f}..{max(ys):.1f}), Z=({min(zs):.1f}..{max(zs):.1f})")

# Rank loose components by vertex count
loose.sort(key=lambda o: len(o.data.vertices), reverse=True)
main_a = loose[0]
for o in loose[1:]:
    bpy.data.objects.remove(o, do_unlink=True)

main_a.name = "TheA_Pure"
...
print("Exporting pure 'a' GLB to:", OUT_GLB)
...
print('Rendered pure preview to:', RENDER_OUT)
