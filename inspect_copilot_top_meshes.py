...
if obj:
    ...
    loose_objs.sort(key=lambda o: len(o.data.vertices), reverse=True)
    
    print("Top 10 largest loose meshes in Copilot3D:")
    for i, o in enumerate(loose_objs[:10]):
        bbox = o.bound_box
        xs = [b[0] for b in bbox]
        ys = [b[1] for b in bbox]
        zs = [b[2] for b in bbox]
        print(f"  #{i+1} '{o.name}': verts={len(o.data.vertices)}, bounds: X=({min(xs):.2f}..{max(xs):.2f}), Y=({min(ys):.2f}..{max(ys):.2f}), Z=({min(zs):.2f}..{max(zs):.2f})")
